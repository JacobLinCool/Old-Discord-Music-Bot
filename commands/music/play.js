...
const data = new SlashCommandSubcommandBuilder()
    .setName("play")
    .setDescription("播放歌曲 play <search|url>")
...
async function run({ player, interaction }) {
    ...
    if (!res || !res.tracks.length) return interaction.editReply(`❌ ${interaction.user} 無相符之查詢結果`);
    ...
    try {
        ...
        return interaction.editReply(`❌ ${interaction.user} 我加不進語音頻道 QQ`);
    }

    await Promise.all([
        interaction.editReply(`🎧 正在載入${res.playlist ? "播放列表" : "歌曲"}...`),
