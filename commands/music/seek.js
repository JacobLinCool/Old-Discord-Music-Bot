...
async function run({ player, interaction }) {
    ...
    try {
        if (args.length === 1) {
            ...
            return interaction.editReply(`❌ ${interaction.user} 參數只接受 mm:ss 或 ss`);
        }
    } catch {
        return interaction.editReply(`❌ ${interaction.user} 參數只接受 mm:ss 或 ss`);
    }
    ...
    if (time >= queue.current.durationMS)
        return interaction.editReply(`❌ ${interaction.user} 跳轉時間超出歌曲長度 (${queue.current.duration})！ \n可接受參數： mm:ss 或 ss`);

    await queue.seek(time);

    return interaction.editReply(`已跳轉至 ${parseInt(time / 60000)} 分 ${parseInt((time % 60000) / 1000)} 秒`);
}
