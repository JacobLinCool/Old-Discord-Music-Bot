...
async function run({ game, player, interaction }) {
    ...
    if (!queue.previousTracks || !queue.previousTracks[1])
        return interaction.reply({ content: `❌ ${interaction.user} 找不到前一首播放的歌曲喔`, ephemeral: true });
    if (game[interaction.guildId]) return await interaction.reply(`❌ ${interaction.user} 遊戲進行中，此操作已禁止`);

    const title = queue.previousTracks[queue.previousTracks.length - 2].title;
    await queue.back();
